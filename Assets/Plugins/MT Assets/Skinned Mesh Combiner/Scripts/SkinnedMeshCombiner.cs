    ...
    public class SkinnedMeshCombiner : MonoBehaviour
    {
        ...
        public class CustomInspector : UnityEditor.Editor
        {
            ...
            void Tab_Merge(SkinnedMeshCombiner script)
            {
                ...
                if (script.resultMergeGameObject == null)
                {
                    ...
                    Texture2D removeItemIcon = (Texture2D)AssetDatabase.LoadAssetAtPath("Assets/Plugins/MT Assets/Skinned Mesh Combiner/Editor/Images/Remove.png", typeof(Texture2D));
            ...
            void MergeParms_JustMaterialColorsParams(SkinnedMeshCombiner script)
            {
                ...
                if (script.justMaterialColorsParams.materialToUse == null)
                    ...
                    {
                        SkinnedMeshRenderer[] renderers = script.gameObject.GetComponentsInChildren<SkinnedMeshRenderer>(true);
                        if (renderers.Length > 0)
                        {
                            Material newMaterial = new Material(renderers[0].sharedMaterials[0].shader);
                            ...
                            script.justMaterialColorsParams.materialToUse = (Material)AssetDatabase.LoadAssetAtPath(materialNameToSave, typeof(Material));
                            EditorUtility.SetDirty(script);
                            EditorGUIUtility.PingObject(script.justMaterialColorsParams.materialToUse);
                        }
                    }
                EditorGUILayout.EndHorizontal();
                ...
                script.justMaterialColorsParams.textureCompression = (TextureCompression)EditorGUILayout.EnumPopup(new GUIContent("Texture Compression",
                    "Texture compression reduces the size (in memory) of Textures generated during merge, and works in both, Runtime and Editor, on any platform. Additionally, Texture compression makes Textures use less graphics memory and more faster to render (because they will use less memory capacity/bandwidth).\n\nDisabled - It does not exert any type of compression.\n\nNormal Quality - Faster, lower quality compression, may leave artifacts in compressed textures.\n\nHigh Quality - Slower, higher quality compression.\n\nNOTE: Keep in mind that compression may increase merging time. Additionally, it will be ignored at Runtime if the hardware does not support Texture compression.\n\nNOTE: The compression format used in merges made in the Editor is DXT1/DXT5, but if the Editor has the build platform active, being Android or iOS, the compression format will be ETC/EAC. In runtime, Android and iOS devices have ETC/EAC as their main supported format and do not support the DXT1/DXT5 format.\n\nNOTE: Merges made in Runtime will use the ETC/EAC compression format on Android and iOS platforms, and DX1/DXT5 on other platforms. Merges made in the Editor will have their textures automatically managed by the Skinned Mesh Combiner and will use either the ETC/EAC or DX1/DXT5 formats, depending on each texture and the active build platform in the Editor."),
                    script.justMaterialColorsParams.textureCompression);
            ...
            void MergeParms_AllInOneParams(SkinnedMeshCombiner script)
            {
                ...
                if (script.allInOneParams.materialToUse == null)
                    if (GUILayout.Button("+", GUILayout.Width(24), GUILayout.Height(16)))
                    {
                        ...
                        if (renderers.Length > 0)
                        {
                            ...
                            AssetDatabase.SaveAssets();
                            AssetDatabase.Refresh();
                            script.allInOneParams.materialToUse = (Material)AssetDatabase.LoadAssetAtPath(materialNameToSave, typeof(Material));
                            EditorUtility.SetDirty(script);
                            EditorGUIUtility.PingObject(script.allInOneParams.materialToUse);
                        }
                    }
                EditorGUILayout.EndHorizontal();
                ...
                script.allInOneParams.materialColorSupport = (bool)EditorGUILayout.Toggle(new GUIContent("Material Color Support",
                    "If this option is enabled, Skinned Mesh Combiner will try to find the Main Texture color in the Materials and take it into account in the Atlas generation process.\n\nYou will also need to provide the name of the property on which the shaders save the texture Color in \"Material Color Settings\". Usually the most used name is \"_Color\".\n\nKeep in mind that this function can increase the time taken to process the merge."),
                    script.allInOneParams.materialColorSupport);
                if (script.allInOneParams.materialColorSupport == true)
                {
                    EditorGUI.indentLevel += 1;
                    script.allInOneParams.materialColorPropertyToFind = script.DrawDropDownOfProperties("Find Colors In",
                        "The name of the shader property, which is responsible for storing the main texture Colors, in the material of its meshes. The Skinned Mesh Combiner will use the property here reported to fetch the Color on each material in each mesh. Usually the name used by most shaders is \"_Color\", but if any of your shaders have a different name, you can enter it here.\n\nIf the Skinned Mesh Combiner can not find the Colors in the mesh material, the color used will be white.",
                        "_Color", "(Color Property)", script.allInOneParams.materialColorPropertyToFind, propertiesOfAllMaterialsFounded);
                    EditorGUI.indentLevel -= 1;
                }
                ...
                script.allInOneParams.emissionMapSupport = (bool)EditorGUILayout.Toggle(new GUIContent("Emission Map Support",
                    "If this option is enabled, the Skinned Mesh Combiner will try to look up Emission Map textures in each material of this model and combine them as well.\n\nYou will also need to provide the name of the property on which the shaders save the Emission Map texture map in \"Emission Map Settings\". Usually the most used name is \"_EmissionMap\".\n\nKeep in mind that this function can increase the time taken to process the merge."),
                    script.allInOneParams.emissionMapSupport);
                if (script.allInOneParams.emissionMapSupport == true)
                {
                    EditorGUI.indentLevel += 1;
                    script.allInOneParams.emissionMapPropertyToFind = script.DrawDropDownOfProperties("Find Text. Maps In",
                        "The name of the shader property, which is responsible for storing the Emission Map texture, in the material of its meshes. The Skinned Mesh Combiner will use the property here reported to fetch the Emission Map texture on each material in each mesh. Usually the name used by most shaders is \"_EmissionMap\", but if any of your shaders have a different name, you can enter it here.\n\nIf the Skinned Mesh Combiner can not find the Emission Map in the mesh material, it will be without Emission Map after merging.",
                        "_EmissionMap", "(Emission Map Property)", script.allInOneParams.emissionMapPropertyToFind, propertiesOfAllMaterialsFounded);

                    script.allInOneParams.emissionMapPropertyToInsert = script.DrawDropDownOfProperties("Apply Merged Map In",
                        "The name of the shader property, which will be responsible for storing the texture of Emission Map atlas, in the COMBINED mesh material. The Skinned Mesh Combiner will use the property here informed to apply the Emission atlas map texture in the final material after the merge. Normally the name used by most shaders (including the standard pre-built shader) is \"_EmissionMap\", but if you have defined a custom shader and it has a different property name, you can enter it here.\n\nIf the Skinned Mesh Combiner can not find the Emission Map in the final material of the mesh merge, it will be without Emission Map after merging.",
                        "_EmissionMap", "(Emission Map Property)", script.allInOneParams.emissionMapPropertyToInsert, propertiesOfMaterialToUse);

                    script.allInOneParams.emissionColorPropertyToFind = script.DrawDropDownOfProperties("Find Colors In",
                        "The name of the shader property, which is responsible for storing the Emission Colors, in the material of its meshes. The Skinned Mesh Combiner will use the property here reported to fetch the Color on each material in each mesh. Usually the name used by most shaders is \"_EmissionColor\", but if any of your shaders have a different name, you can enter it here.\n\nIf the Skinned Mesh Combiner can not find the Colors in the mesh material, the color used will be black.",
                        "_EmissionColor", "(Color Property)", script.allInOneParams.emissionColorPropertyToFind, propertiesOfAllMaterialsFounded);

                    script.allInOneParams.emissionLightType = (MaterialGlobalIllumination)EditorGUILayout.EnumPopup(new GUIContent("Global Illumination",
                        "Defines the type of Global Illumination to be used by the Emission, in the Material resulting from the merge."),
                        script.allInOneParams.emissionLightType);
                    EditorGUI.indentLevel -= 1;
                }
                ...
                script.allInOneParams.textureCompression = (TextureCompression)EditorGUILayout.EnumPopup(new GUIContent("Texture Compression",
                    "Texture compression reduces the size (in memory) of Textures generated during merge, and works in both, Runtime and Editor, on any platform. Additionally, Texture compression makes Textures use less graphics memory and more faster to render (because they will use less memory capacity/bandwidth).\n\nDisabled - It does not exert any type of compression.\n\nNormal Quality - Faster, lower quality compression, may leave artifacts in compressed textures.\n\nHigh Quality - Slower, higher quality compression.\n\nNOTE: Keep in mind that compression may increase merging time. Additionally, it will be ignored at Runtime if the hardware does not support Texture compression.\n\nNOTE: The compression format used in merges made in the Editor is DXT1/DXT5, but if the Editor has the build platform active, being Android or iOS, the compression format will be ETC/EAC. In runtime, Android and iOS devices have ETC/EAC as their main supported format and do not support the DXT1/DXT5 format.\n\nNOTE: Merges made in Runtime will use the ETC/EAC compression format on Android and iOS platforms, and DX1/DXT5 on other platforms. Merges made in the Editor will have their textures automatically managed by the Skinned Mesh Combiner and will use either the ETC/EAC or DX1/DXT5 formats, depending on each texture and the active build platform in the Editor."),
                    script.allInOneParams.textureCompression);
                ...
                if (Application.isPlaying == false)
                {
                    ...
                    if (script.resultMergeGameObject != null)
                    {
                        ...
                        if (script.savePrefabOfMerge == true)
                        {
                            ...
                            {
                                EditorGUIUtility.PingObject(AssetDatabase.LoadAssetAtPath("Assets/Plugins/MT Assets/_AssetsData/Assets/Prefabs", typeof(object)));
                            }
                            EditorGUILayout.EndVertical();
                            EditorGUILayout.EndHorizontal();
                            GUILayout.Space(2);
            ...
            foreach (Material mat in materials)
            {
                for (int i = 0; i < ShaderUtil.GetPropertyCount(mat.shader); i++)
                {
                    if (dictionary.ContainsKey(ShaderUtil.GetPropertyName(mat.shader, i)) == false)
                    {
                        ...
                        if (propertyType == ShaderUtil.ShaderPropertyType.TexEnv || propertyType == ShaderUtil.ShaderPropertyType.Color)
                        {
        ...
        private void DoCombineMeshs_JustMaterialColors()
        {
            ...
            MergePoint:
                ...
                //Compress all textures of all generated altases, if desired, and if is in Runtime...
                //The compression format used, for Runtime, will always be DXT5/ETC2 (because all textures generated by SMC are RGBA32). There will be no conversion work for DXT1/ETC1, as compression to DXT5/ETC2 is ALWAYS faster.
                if (justMaterialColorsParams.textureCompression != TextureCompression.Disabled && Application.isPlaying == true)
                {
                    //Get the target compression quality
                    bool quality = false;
                    if (justMaterialColorsParams.textureCompression == TextureCompression.NormalQuality)
                        quality = false;
                    if (justMaterialColorsParams.textureCompression == TextureCompression.HighQuality)
                        quality = true;

                    //Compress all textures of atlas
                    atlasGenerated.colorAtlas.Compress(quality);
                }
                ...
                //The SMC Compressed Textures manager, in Editor, will compress the textures in DXT1/ETC1 or DXT5/ETC2 according to active platform and texture type!
                string compressionFlag = "";
                if (justMaterialColorsParams.textureCompression == TextureCompression.Disabled) { compressionFlag = "SMC-UMin"; }
                if (justMaterialColorsParams.textureCompression == TextureCompression.NormalQuality) { compressionFlag = "SMC-NMin"; }
                if (justMaterialColorsParams.textureCompression == TextureCompression.HighQuality) { compressionFlag = "SMC-HMin"; }
                //Save data as asset
                if (saveDataInAssets == true)
                    SaveAssetAsFile("Meshes", smrender.sharedMesh, this.gameObject.name, "asset");
                if (saveDataInAssets == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.colorAtlas, this.gameObject.name + " (ColorAtlas) " + compressionFlag, "asset");
                if (saveDataInAssets == true)
        ...
        private void DoCombineMeshs_AllInOne()
        {
                ...
                foreach (SkinnedMeshRenderer meshRender in skinnedMeshesToMerge)
                {
                    ...
                    for (int i = 0; i < meshRender.sharedMesh.subMeshCount; i++)
                    {
                        ...
                        if (isTiledTexture(boundDataOfUv) == true)
                        {
                            ...
                            if (allInOneParams.emissionMapSupport == true)
                                thisTextureAndSubMesh.emissionMap = GetValidatedCopyOfTexture(thisTextureAndSubMesh.material, allInOneParams.emissionMapPropertyToFind, thisTextureAndSubMesh.mainTextureResolutionWithEdges.x, thisTextureAndSubMesh.mainTextureResolutionWithEdges.y, boundDataOfUv, TextureType.EmissionMap, true, progressOfThisMeshRead);
            ...
            MergePoint:
                ...
                //Compress all textures of all generated altases, if desired, and if is in Runtime...
                //The compression format used, for Runtime, will always be DXT5/ETC2 (because all textures generated by SMC are RGBA32). There will be no conversion work for DXT1/ETC1, as compression to DXT5/ETC2 is ALWAYS faster.
                if (allInOneParams.textureCompression != TextureCompression.Disabled && Application.isPlaying == true)
                {
                    //Get the target compression quality
                    bool quality = false;
                    if (allInOneParams.textureCompression == TextureCompression.NormalQuality)
                        quality = false;
                    if (allInOneParams.textureCompression == TextureCompression.HighQuality)
                        quality = true;

                    //Compress all textures of atlas
                    atlasGenerated.mainTextureAtlas.Compress(quality);
                    if (allInOneParams.metallicMapSupport == true)
                        atlasGenerated.metallicMapAtlas.Compress(quality);
                    if (allInOneParams.specularMapSupport == true)
                        atlasGenerated.specularMapAtlas.Compress(quality);
                    if (allInOneParams.normalMapSupport == true)
                        atlasGenerated.normalMapAtlas.Compress(quality);
                    if (allInOneParams.normalMap2Support == true)
                        atlasGenerated.normalMap2Atlas.Compress(quality);
                    if (allInOneParams.heightMapSupport == true)
                        atlasGenerated.heightMapAtlas.Compress(quality);
                    if (allInOneParams.occlusionMapSupport == true)
                        atlasGenerated.occlusionMapAtlas.Compress(quality);
                    if (allInOneParams.detailAlbedoMapSupport == true)
                        atlasGenerated.detailMapAtlas.Compress(quality);
                    if (allInOneParams.detailMaskSupport == true)
                        atlasGenerated.detailMaskAtlas.Compress(quality);
                    if (allInOneParams.emissionMapSupport == true)
                        atlasGenerated.emissionMapAtlas.Compress(quality);
                }
                ...
                if (allInOneParams.emissionMapSupport == true)
                    ApplyAtlasInPropertyOfMaterial(smrender.sharedMaterials[0], allInOneParams.emissionMapPropertyToInsert, atlasGenerated.emissionMapAtlas);
                ...
                //The SMC Compressed Textures manager, in Editor, will compress the textures in DXT1/ETC1 or DXT5/ETC2 according to active platform and texture type!
                string compressionFlag = "";
                if (allInOneParams.textureCompression == TextureCompression.Disabled) { compressionFlag = "SMC-UMin"; }
                if (allInOneParams.textureCompression == TextureCompression.NormalQuality) { compressionFlag = "SMC-NMin"; }
                if (allInOneParams.textureCompression == TextureCompression.HighQuality) { compressionFlag = "SMC-HMin"; }
                //Save data as asset
                if (saveDataInAssets == true)
                    SaveAssetAsFile("Meshes", smrender.sharedMesh, this.gameObject.name, "asset");
                if (saveDataInAssets == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.mainTextureAtlas, this.gameObject.name + " (MainTexture) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.metallicMapSupport == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.metallicMapAtlas, this.gameObject.name + " (MetallicMap) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.specularMapSupport == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.specularMapAtlas, this.gameObject.name + " (SpecularMap) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.normalMapSupport == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.normalMapAtlas, this.gameObject.name + " (NormalMap) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.normalMap2Support == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.normalMap2Atlas, this.gameObject.name + " (NormalMap2x) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.heightMapSupport == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.heightMapAtlas, this.gameObject.name + " (HeightMap) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.occlusionMapSupport == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.occlusionMapAtlas, this.gameObject.name + " (OcclusionMap) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.detailAlbedoMapSupport == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.detailMapAtlas, this.gameObject.name + " (DetailMap) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.detailMaskSupport == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.detailMaskAtlas, this.gameObject.name + " (DetailMask) " + compressionFlag, "asset");
                if (saveDataInAssets == true && allInOneParams.emissionMapSupport == true)
                    SaveAssetAsFile("Atlases", atlasGenerated.emissionMapAtlas, this.gameObject.name + " (EmissionMap) " + compressionFlag, "asset");
                if (saveDataInAssets == true)
                    SaveAssetAsFile("Materials", smrender.sharedMaterials[0], this.gameObject.name, "mat");
        ...
        private void DoCombineMeshs_OnlyAnima2dMeshs()
        {
            ...
            try
            {
                ...
                //Generate the atlas
                Texture2D atlasTexure = new Texture2D(16, 16, TextureFormat.RGBA32, false, false);
        ...
        private void DoUndoCombineMeshs(bool runMonoIl2CppGc, bool runUnityGc, bool errorOcurred)
        {
            ...
            foreach (string str in resultMergeAssetsSaved)
            {
                ...
                if (AssetDatabase.LoadAssetAtPath(str.Replace(".asset", " RAW.asset"), typeof(object)) != null)
                    AssetDatabase.DeleteAsset(str.Replace(".asset", " RAW.asset"));
        ...
        {
            //This class store a atlas data
            public Texture2D colorAtlas = new Texture2D(16, 16, TextureFormat.RGBA32, true, false);
            public Rect[] atlasRects = new Rect[0];
        ...
        private void SaveAssetAsFile(string folderNameToSave, UnityEngine.Object assetToSave, string fileName, string fileExtension)
        {
            ...
            if (!AssetDatabase.IsValidFolder("Assets/Plugins/MT Assets/_AssetsData"))
                AssetDatabase.CreateFolder("Assets/Plugins/MT Assets", "_AssetsData");
            if (!AssetDatabase.IsValidFolder("Assets/Plugins/MT Assets/_AssetsData/" + folderNameToSave))
                AssetDatabase.CreateFolder("Assets/Plugins/MT Assets/_AssetsData", folderNameToSave);
            ...
            //Save the asset
            string fileDirectory = "Assets/Plugins/MT Assets/_AssetsData/" + folderNameToSave + "/" + fileName + " (" + dateNowStr + ")." + fileExtension;
        ...
        private void SaveMergeAsPrefab(string folderNameToSave, string prefabName, GameObject targetGo)
        {
            ...
            //Save the asset
            string fileDirectory = "Assets/Plugins/MT Assets/_AssetsData/" + folderNameToSave + "/" + prefabName + ".prefab";
            ...
            if (AssetDatabase.LoadAssetAtPath(fileDirectory, typeof(GameObject)) == null)
            {
                ...
                LaunchLog("The prefab \"" + name + "\" was created in your project files! The prefab generated with the desired name, can be found at \"Assets/Plugins/MT Assets/_AssetsData/Prefabs\"", LogTypeOf.Log);
            }
#endif
        }
        ...
        private Texture2D GetValidatedCopyOfTexture(Material materialToFindTexture, string propertyToFindTexture, int widthOfCorrespondentMainTexture, int heightOfCorrespondentMainTexture, TexturesSubMeshes.UvBounds boundsUvValues, TextureType textureType, bool showProgress, float progress)
        {
            ...
            if (textureType == TextureType.MainTexture && allInOneParams.materialColorSupport == true)
            {
                //Try to get the color of the material
                Color materialColor = Color.white;
                materialToFindTexture.EnableKeyword(allInOneParams.materialColorPropertyToFind);

                //If found the property of color
                if (materialToFindTexture.HasProperty(propertyToFindTexture) == true)
                    materialColor = materialToFindTexture.GetColor(allInOneParams.materialColorPropertyToFind);
                //If not found the property of texture
                if (materialToFindTexture.HasProperty(propertyToFindTexture) == false)
                    LaunchLog("It was not possible to find the Material Color stored in property \"" + allInOneParams.materialColorPropertyToFind + "\" of material \"" + materialToFindTexture.name + "\", so this Color was replaced by a WHITE.", LogTypeOf.Warning);
            ...
            if (textureType == TextureType.EmissionMap && allInOneParams.emissionMapSupport == true)
            {
                //Try to get the color of the emission
                Color emissionColor = Color.white;
                materialToFindTexture.EnableKeyword(allInOneParams.emissionColorPropertyToFind);

                //If found the property of color
                if (materialToFindTexture.HasProperty(propertyToFindTexture) == true)
                    emissionColor = materialToFindTexture.GetColor(allInOneParams.emissionColorPropertyToFind);
                //If not found the property of texture
                if (materialToFindTexture.HasProperty(propertyToFindTexture) == false)
                    LaunchLog("It was not possible to find the Emission Color stored in property \"" + allInOneParams.emissionColorPropertyToFind + "\" of material \"" + materialToFindTexture.name + "\", so this Color was replaced by a BLACK.", LogTypeOf.Warning);
            ...
            try
            {
                ...
                thisTexture = new Texture2D(texWidth, texHeight, TextureFormat.RGBA32, false, false);
                ...
                //If pink normal maps fix is enabled, and if this is a normal map, try to get colors using different decoding (if have a compression format that uses different channels to store colors)
                if (allInOneParams.pinkNormalMapsFix == true && textureType == TextureType.NormalMap)
        ...
        private AtlasData CreateAllAtlas(List<TexturesSubMeshes> copyiedTextures, int maxResolution, int paddingBetweenTextures, bool showProgress)
        {
            ...
            //Create the emission map if is desired
            if (allInOneParams.emissionMapSupport == true)
            {
                if (showProgress == true)
                    ShowProgressBar("Creating Emission Map Atlas...", true, 1.0f);
                texturesToUse.Clear();
                foreach (TexturesSubMeshes item in copyiedTextures)
                    texturesToUse.Add(item.emissionMap);
                atlasData.emissionMapAtlas.PackTextures(texturesToUse.ToArray(), paddingBetweenTextures, maxResolution);
            }
        ...
        private void ApplyAtlasInPropertyOfMaterial(Material targetMaterial, string propertyToInsertTexture, Texture2D atlasTexture)
        {
            ...
            if (targetMaterial.HasProperty(propertyToInsertTexture) == true)
            {
                ...
                if (propertyToInsertTexture == "_EmissionMap" && targetMaterial.IsKeywordEnabled("_EMISSION") == false && allInOneParams.emissionMapSupport == true)
                    targetMaterial.EnableKeyword("_EMISSION");
                if (propertyToInsertTexture == "_EmissionMap" && allInOneParams.emissionMapSupport == true)
                {
                    if (allInOneParams.emissionLightType == MaterialGlobalIllumination.Baked)
                        targetMaterial.globalIlluminationFlags = MaterialGlobalIlluminationFlags.BakedEmissive;
                    if (allInOneParams.emissionLightType == MaterialGlobalIllumination.Realtime)
                        targetMaterial.globalIlluminationFlags = MaterialGlobalIlluminationFlags.RealtimeEmissive;
                    if (allInOneParams.emissionLightType == MaterialGlobalIllumination.None)
                        targetMaterial.globalIlluminationFlags = MaterialGlobalIlluminationFlags.None;
                    targetMaterial.SetColor(allInOneParams.emissionColorPropertyToFind, Color.white);
                }
            }
